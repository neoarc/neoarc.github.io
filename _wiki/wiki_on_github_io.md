...
* 우선, 종립님의 [[https://johngrib.github.io/wiki/my-wiki/]]{Vimwiki + Jekyll + Github.io로 나만의 위키를 만들자}를 읽었다고 가정한다.
* 종립님의 글에서 매우 자세히 구축 방법을 설명하고 있지만, 리눅스 환경에서의 구축 방법이므로 Windows 사용자가 따라하기엔 알아들을 수 없는 내용 투성이였다.
* 이런저런 삽질 끝에 어느정도 안정적(?)으로 사용할 수 있게 되었기에 이 글에서는 그 내용을 정리한다.
...
## 종립님의 스켈레톤을 Fork하여 내 Github Page에 커밋한다.
...
- 후에 알았지만, 이것들은 Jekyll 홈페이지만 한번 살펴보면 알 수 있는 내용들이다.
...
- gVim을 다시 실행해보면, 메뉴에 Vimwiki가 보일것이다.
	- 대략 첫번째 메뉴를 눌러보면, 위키 홈 폴더를 생성하겠다고 한다.
		...
		- 기본 경로로 만들면 c:/Users/{윈도우 계정명}/ 의 하위에 생성될 것이다. (나의 경우 이 경로를 좋아하지 않으므로 별도의 경로에 만들었다.)
...
set encoding=utf-8
	...
	- 이렇게 하고나면
		- gVim의 기본 언어가 영어로 변경되고
		- 파일 인코딩이 기본 utf-8로 지정된다.

# 대강 쓸수는 있게 되었다.

- 여기까지 읽은 분이 혹시 있다면 이쯤에서 대강 감을 잡았으리라 생각된다.
- 아직 모든 기능이 완벽하게 동작하지는 않으므로 좀 더 손봐줘야 하지만, 급한 일은 아니다. ~~시간 나는 대로 적어보자.~~

# 아직 남은 할 일들

- 다음 링크의 글에 별도로 정리한다: [[my_wiki_todolist]]{아직 남은 할 일들}
